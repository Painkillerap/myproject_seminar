...
def store_index(request):
    ...
    logger.info(text)
    return render(request, 'hwapp_2/store_index.html', {'text': text})


def clients_all(request):
    clients = Client.objects.all()
    logger.info('Index page "Client ALL" accessed')
    return render(request, 'hwapp_2/clients_all.html', {'clients': clients})
...
def order(request, order_id):
    ...
    return render(request, 'hwapp_2/order.html', {'order': order, 'products_in_order': products_in_order})
...
def orders_by_client(request, client_id):
    client = get_object_or_404(Client, pk=client_id)
    orders = Order.objects.filter(client__pk=client_id)
    return render(request, 'hwapp_2/orders_no_sort.html', {'orders': orders, 'client': client})
...
def orders_by_client_sort(request, client_id):
    ...
    return render(request, 'hwapp_2/orders_sort.html', {
        'orders_7': orders_7,
        'orders_30': orders_30,
